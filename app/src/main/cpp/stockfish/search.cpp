...
void Search::setJavaGameRef(JavaGameRef* game) {
    gameRef = game;
}
...
void MainThread::search() {
  ...
  if (bestThread != this) {
      string t = UCI::pv(bestThread->rootPos, bestThread->completedDepth, -VALUE_INFINITE,
              VALUE_INFINITE);
//      sync_cout << t << sync_endl;

//      bestmove += t;
  }

  string s = "bestmove " + UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());
//  sync_cout << s;
  bestmove += s;

  if (bestThread->rootMoves[0].pv.size() > 1 || bestThread->rootMoves[0].extract_ponder_from_tt(rootPos)) {
//      string t = " ponder " + UCI::move(bestThread->rootMoves[0].pv[1], rootPos.is_chess960());
//      std::cout << t;
//      bestmove += t;
  }
    JavaVM *javaVM = gameRef->javaVM;
    JNIEnv *env;
    ...
    jmethodID statusId = env->GetMethodID(gameRef->gameClass,
                                          "handleStockfishBestMove",
                                          "(Ljava/lang/String;)V");

    char * tab = new char [bestmove.length()+1];
    strcpy (tab, bestmove.c_str());
    sendJavaMsg(env, gameRef->gameObject, statusId, tab);
